"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OtpService = exports.IOtpService = void 0;
const common_1 = require("@nestjs/common");
const otp_repository_1 = require("../repositories/otp.repository");
exports.IOtpService = Symbol('IOtpService');
let OtpService = class OtpService {
    constructor(otpRepository) {
        this.otpRepository = otpRepository;
    }
    create(createOtpDto, options) {
        return this.otpRepository.create(createOtpDto, options);
    }
    update(conditions, payload, options) {
        return this.otpRepository.findOneAndUpdate(conditions, payload, options);
    }
    findByCode(code) {
        return this.otpRepository.findOne({
            conditions: {
                code
            }
        });
    }
    findByUserIdAndRole(userId, role) {
        return this.otpRepository.findOne({
            conditions: {
                userId,
                role
            }
        });
    }
    async clearOtp(code) {
        await this.otpRepository.model.deleteOne({
            code
        });
        return;
    }
};
exports.OtpService = OtpService;
exports.OtpService = OtpService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(otp_repository_1.IOtpRepository)),
    __metadata("design:paramtypes", [Object])
], OtpService);
//# sourceMappingURL=otp.service.js.map